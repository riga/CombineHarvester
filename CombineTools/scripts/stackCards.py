...
def stack_cards(input_cards, output_card, output_shapes, analysis_name="analysis", era="13TeV",
    ...
    # loop through cards and set input data, with the expanded path being the key
    ...
    for syst_name, syst_data in syst_map.items():
        for process_name, process_data in syst_data.items():
            ...
            if syst_type in ["lnN", "lnU"]:
                ...
                # via the python bindings, so just use the configuration of the parameter given the
                # first datacard that contains such a parameter
...
def stack_shapes(input_data, syst_map, shape_patterns, bin_name, mass="125"):
    """
    Takes the *input_data* and *syst_map* objects generated by :py:func:`stack_shapes`, which
    ...
    stored systematically varied histograms. In these patters, the variables ``"%BIN"``,
    ``"%PROCESS"``, ``"%SYSTEMATIC"`` and ``"%MASS"`` are replaced with the values applying for a
    ...
    def check_bin_edges(hists, epsilon=1e-5):
        ...
        for i, hist in enumerate(hists):
            ...
            if not edges:
                ...
                # compare overall number of bins
